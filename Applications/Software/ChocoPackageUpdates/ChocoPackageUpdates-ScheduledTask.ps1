...
# The Script runs a local script, which calls Chocolately to update all out-dated packages (that were originally installed by Chocolately)
# The Script will run only at specified schedules, in order stagger updates and group/stage pilot/test/UAT machines.
# The Script can run at computer startup to ensure that all apps are closed and that the update will be succesful
...
# Variables
$STPath = "Bennell IT"
$STName = "Bennell IT - Choco Package Updates - $ReleaseChannel"
$ScriptsFolder = "$env:ProgramFiles\bennellit\AppPatching-$ReleaseChannel"
$PSADTFolder = "Files"
...
# Create bennell it Scripts folder in case it doesn't already exist
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Author>AD\SB</Author>
    <Description>The Script runs a local script (PowerShell App Deployment Toolkit aka PSADT), which calls Chocolatey, on scheduled release channel basis (Monthly or Quarterly). Choco will then upgrade all apps that it manages.</Description>
    <URI></URI>
  </RegistrationInfo>
  <Triggers>
    <CalendarTrigger>
...
Try {
    ...
    If($ReleaseChannel -ne 'Monthly'){
      ...
      Write-Log -LogFile $sLogFile -Value "Removing random delay from Script as ReleaseChannel: [$ReleaseChannel] specified" -Component $Section -Severity 1
